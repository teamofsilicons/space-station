...
fn seed(org: &str, other: &str) -> Seed {
    serde_json::from_value(json!({
        ...
        "carbons": [
            {"carbon_id": "c:alice", "name": "Alice", "memberships": {
                org: {"org_role": "owner", "tags": ["tech"]},
                other: {"org_role": "member", "tags": ["sales"]},
            }},
            {"carbon_id": "c:bob", "name": "Bob", "memberships": {}},
        ],
        "silicons": [{"silicon_id": "si:bot", "org_id": org, "name": "Bot", "token": STK, "tags": ["ops"]}],
    }))
...
/// Plays the browser as `carbon`: IAM selects the requested org after the backend redirects;
/// the stub signs them in (`?as=`) and the callback exchanges its `?slt=` and lands on `next`.
...
fn member_row(
    ...
    let kind = space_station_backend::iam::Kind::of(actor).unwrap().as_str();
    ...
    json!({
        ...
        "principal": {"principal_id": actor, "public_id": actor, "type": kind, "display_name": actor, "status": "active"},
...
fn event(event_type: &str, members: Vec<Value>) -> Value {
    let aggregate = members.first().map_or_else(|| json!(Uuid::now_v7()), |m| m["resource"]["id"].clone());
    json!({"spec_version": "1.0", "event_id": Uuid::now_v7(), "event_type": event_type, "occurred_at": Utc::now().to_rfc3339(),
           "organization_id": Uuid::now_v7(), "aggregate": {"type": "organization_membership", "id": aggregate, "version": 1},
...
async fn the_whole_station_end_to_end() {
    ...
    assert_eq!(me, json!({"kind": "carbon", "id": "c:alice", "org": org, "tags": ["tech"]}), "tags from the snapshot");
    ...
    authorize.query_pairs_mut().append_pair("as", "c:alice").append_pair("org_id", &org);
    let res = api.http.get(authorize).send().await.unwrap();
    let callback = res.headers()["location"].to_str().unwrap().to_owned();
    let res = api.http.get(&callback).send().await.unwrap();
    ...
    assert_eq!(terminal.ok(Method::GET, "/me", None).await["id"], "c:alice");
    ...
    let cat = iam.carbon_bearer("c:alice").await;
    ...
    assert_eq!(tables[0]["access"], json!(["@c:alice"]), "the creator is appended");
    assert_eq!(tables[0]["records"], 0);
    let emptied = api.ok(Method::PUT, &format!("{orgs}/tables/orders"), Some(json!({"access": []}))).await;
    assert_eq!(emptied["access"], json!(["@c:alice"]), "an access edit cannot leave the table unreachable");
    ...
    assert_eq!((window["version"].clone(), window["access"].clone()), (Value::Null, json!(["@c:alice"])));
    let kept = api.ok(Method::PUT, &format!("{orgs}/windows/{id}"), Some(json!({"access": []}))).await;
    assert_eq!(kept["access"], json!(["@c:alice"]), "nor can it leave the window unreachable");
    ...
    assert_eq!((version["name"].as_str(), version["created_by"].as_str()), (Some("v1"), Some("c:alice")));
    ...
    let bot_membership = format!("{bot}[{org}]");
    let bot_row = |tags: &[&str], version| member_row(&org, "Test Org", &bot, &bot_membership, "active", tags, version);
    let tagged = event("organization.silicon.updated.v1", vec![bot_row(&["ops"], 2)]);
    assert_eq!(
        deliver(&api, "/api/iam/webhook", WEBHOOK_SECRET, now, &tagged.to_string()).await,
        401,
        "a production envelope cannot enter a testing database"
    );
    let tagged = test_envelope(TEST_KEY, &tagged).to_string();
    ...
    let stale =
        test_envelope(TEST_KEY, &event("organization.membership.updated.v1", vec![bot_row(&[], 1)])).to_string();
    assert_eq!(
    ...
    let alice_rows = vec![
        member_row(&org, "Test Org", "c:alice", &format!("c:alice[{org}]"), "active", &["tech"], 2),
        member_row(&other, "Second Org", "c:alice", &format!("c:alice[{other}]"), "active", &["sales"], 2),
    ];
    ...
    let (status, _) =
        iam.app_form("/api/v1/app-auth/tokens", &format!("app_id=spacestation&refresh_token={ort}")).await;
    ...
    let gone = member_row(&org, "Test Org", "c:alice", &format!("c:alice[{org}]"), "removed", &[], 3);
    let removed = test_envelope(TEST_KEY, &event("organization.membership.removed.v1", vec![gone])).to_string();
    ...
    drop(client);
    for test_key in [Some("KkKkKkKkKkKkKkKkKkKkKkKkKkKkKkKk".to_owned()), None] {
        let mut mismatched = config(port, &origin, iam_addr);
        mismatched.iam_test_key = test_key;
        let Err(error) = space_station_backend::store::Store::connect(&mismatched).await else {
            panic!("a populated testing database must reject a different IAM world");
        };
        assert!(error.to_string().contains("IAM testing environment does not match"), "{error}");
    }
    app.stop().await;
}
